...
function EnhancedDashboardContent({ loading, onRefresh }: any) {
  const [dashboardData, setDashboardData] = useState<DashboardData>(null);
  const [dataLoading, setDataLoading] = useState(true);
  const [userRole, setUserRole] = useState<"super_admin" | "admin" | "user">(
    "user"
  );
  ...
  const calculatePercentageChange = (
    type: "deposits" | "spent" | "allocated" | "projects"
  ) => {
  ...
  const getChartData = () => {
    ...
    if (filteredData.length < 6) {
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const targetMonthKey = `${targetYear}-${String(targetMonth).padStart(
          2,
          "0"
        )}`;

        const existingData = filteredData.find(
          (item) => item.month === targetMonthKey
        );
  ...
  const totalFinancialValue = financialData.reduce(
    (sum, d) => sum + normalizeAmount(d.value),
    0
  );
  ...
  const displayPercentage =
    totalFinancialValue > 0
      ? Math.round(
          (normalizeAmount(activeSegment?.value ?? totalFinancialValue) /
            totalFinancialValue) *
            100
        )
      : 0;
  ...
  const getPeriodTotal = () => {
    ...
    if (isAdmin) {
      return normalizeAmount(
        (dashboardData as AdminDashboardData)?.stats.total_deposits
      );
    } else {
      return normalizeAmount(
        (dashboardData as UserDashboardData)?.stats.total_allocated
      );
    }
  };
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-4 sm:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <div className="relative h-56 w-full bg-secondary rounded-2xl p-6 shadow-2xl text-white flex flex-col justify-between overflow-hidden">
              ...
              <div className="relative">
                ...
                {showPeriodDropdown && (
                  <div className="absolute right-0 mt-2 w-40 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-20">
                    {(["all", "week", "month", "year"] as const).map(
                      (period) => (
                        <button
                          key={period}
                          onClick={() => {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-secondary rounded-lg border border-gray-200 p-4">
              ...
              <div>
                ...
                <h3 className="text-lg font-bold text-primary">
                  {isLoading ? (
                    ...
                    formatCurrency(
                      (dashboardData as AdminDashboardData)?.stats
                        .total_deposits -
                        (dashboardData as AdminDashboardData)?.stats
                          .total_allocated || 0
                    )
                    ...
                    formatCurrency(
                      (dashboardData as UserDashboardData)?.stats
                        .remaining_balance || 0
                    )
                  )}
                </h3>
            ...
            <p className="text-xs text-white">Unallocated funds</p>
          </div>
          ...
          <div className="bg-secondary rounded-lg border border-gray-200 p-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <p className="text-xs text-white">Total Expense</p>
                <h3 className="text-lg font-bold text-primary">
                  {isLoading ? (
                    <div className="h-5 w-24 bg-gray-200 rounded animate-pulse"></div>
                  ) : (
            ...
            <p className="text-xs text-white">Total spending</p>
          </div>
        </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="space-y-4">
                {isLoading
                  ...
                  : isAdmin
                  ? (dashboardData as AdminDashboardData)?.recent_allocations
                      ?.slice(0, 4)
                      .map((allocation, index) => (
                        ...
                        >
                          <div className="flex-shrink-0 mt-1">
                            ...
                            {index < 3 && (
                              <div className="w-0.5 h-8 bg-secondary mx-auto mt-1"></div>
                            )}
                          </div>
